...
{
	[SerializeField] BossFish enemyFish;
	Gulpert player;
	Rigidbody2D rb;
	...
	void PassiveState()
	{
		...
		if (Vector3.Distance(transform.position, player.transform.position) < enemyFish.SearchDistance &&
			this.GetComponent<CapsuleCollider2D>().bounds.size.magnitude + player.EatThreshold > player.GetComponent<CapsuleCollider2D>().bounds.size.magnitude)
		{
