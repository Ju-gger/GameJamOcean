...
public class BungaAI : MonoBehaviour
{
	[SerializeField] BungaFish enemyFish;
	Gulpert player;
	Rigidbody2D rb;
	...
	void Awake()
	{
		...
		player = GameObject.FindGameObjectWithTag("Player").GetComponent<Gulpert>();
	}
	...
	void AttackState()
	{
		...
		if (Vector3.Distance(transform.position, player.transform.position) > enemyFish.TravelDistance 
			|| GetComponent<CapsuleCollider2D>().bounds.size.magnitude + player.EatThreshold < player.GetComponent<CapsuleCollider2D>().bounds.size.magnitude)
		{
			ChangeState(PassiveState);
			return;
		}
	...
	void PassiveState()
	{
		...
		/* move towards the patrolPos if within searchDistance AND is bigger than the player fish (IMPLEMENT FOR BIGGER FISH)*/
		if (Vector3.Distance(transform.position, player.transform.position) < enemyFish.SearchDistance &&
			GetComponent<CapsuleCollider2D>().bounds.size.magnitude + player.EatThreshold > player.GetComponent<CapsuleCollider2D>().bounds.size.magnitude)
		{
